...
//   node run-audit.mjs <url> [--repo <chemin>] [--pages N] [--citations] [--render|--no-render] [--logs <fichier>] [--backlinks <csv>] [--cookie "s=..."] [--bearer <token>]
...
// --logs <fichier>: analyse les logs serveur (Combined/NCSA, JSON par ligne, W3C,
//   gzippe ou non). Seule source qui dit ce que les moteurs ont REELLEMENT explore.
// --backlinks <csv>: verifie un a un les liens entrants declares (export Search
//   Console, Ahrefs, Majestic...). Le format est detecte automatiquement.
// Positions reelles: definir PANOPTIC_GSC_TOKEN ou PANOPTIC_GSC_SA (Search Console).
...
let target = null, repoPath = null, maxPages = 1, cookie = null, bearer = null, citations = false, render = null, logFile = null, backlinksFile = null;
const headers = {};
for (let i = 0; i < args.length; i++) {
  ...
  else if (args[i] === "--logs") logFile = args[++i];
  else if (args[i] === "--backlinks") backlinksFile = args[++i];
...
if (!target) { console.error("usage: node run-audit.mjs <url> [--repo <chemin>] [--pages N] [--citations] [--render|--no-render] [--logs <fichier>] [--backlinks <csv>] [--cookie ...] [--bearer ...] [--header 'K: V']"); process.exit(2); }
...
const scan = (t) => recon(t, { repoPath, auth, maxPages, browserScan: render === null ? undefined : render, seoKing: { citations, logFile, backlinksFile, ...(render === false ? { render: false } : {}) } });
