...
const CSS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  @media(max-width:960px){.nlinks{display:none}
    /* Sans les liens de nav, le selecteur de langue est colle au logo (a gauche):
       le menu ancre a droite deborderait du bord gauche. On l'ouvre vers la droite. */
    .langmenu{left:0;right:auto}}
  ...
  @media(max-width:400px){
    ...
    .langsel summary{padding:6px 9px;font-size:11.5px}
  }
  ...
  /* Marges laterales et rythme vertical fluides sur mobile (evite l'exces d'espace
     et le debordement d'en-tete a 320px). */
  @media(max-width:600px){:root{--pad:20px}section{padding:64px 0}}
  @media(max-width:380px){:root{--pad:16px}section{padding:52px 0}}
...
<footer><div class="wrap foot">
  ...
  <div class="flinks"><a href="/console/">${esc(t.footLinks[0])}</a><a href="/comparatif/">${esc(t.footLinks[5])}</a><a href="/docs/">${esc(t.footLinks[4])}</a><a href="/benchmark/">${esc(t.footLinks[1])}</a><a href="/securite/">${esc(t.footLinks[3])}</a><a href="${REPORT_URL}" target="_blank" rel="noopener">${esc(t.footLinks[2])}</a></div>
