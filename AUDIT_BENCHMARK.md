...
- **Action #5 (CLI + MCP + Action GitHub) — FAITE.** `cli/panoptic.mjs` (`panoptic scan --pages/--repo/--json/--fail-on`), `mcp/server.mjs` (outil MCP `panoptic_scan` pour Claude Code/Cursor, JSON-RPC stdio zéro-dep), `action.yml` (audit à chaque deploy). Point d'entrée partagé `engine/audit.mjs` (fin de la duplication CLI/MCP/serveur). Tests `cli/cli.test.mjs` 10/10, CI verte. **Neutralise l'avantage dev-native de squirrelscan** ; différenciateur maintenu (Panoptic lit le code source + vérif déterministe, squirrelscan non). Effet grille : Intégrations 4→7, Défendabilité +1.
- **Action #2 (Comptes utilisateurs — Supabase Auth) — FAITE.** `server/auth.js` (proxy GoTrue côté serveur : signup avec user auto-confirmé via admin API, login par mot de passe, validation JWT avec cache 60 s ; tenant = `u_<userid>` ; la service key ne quitte jamais le serveur), routes `/api/auth/signup|login`, `/api/me`, résolution du principal Bearer-prioritaire (clé API en repli pour CLI/MCP), dashboard avec écran login/signup + portail + déconnexion. **Vérifié live sur Render de bout en bout** : signup→token, /api/me authentifié, login, audits isolés par compte (0 pour un nouveau compte), mauvais mot de passe→401, token bidon→non authentifié. Zéro nouvelle variable d'env (la service key existante suffit), aucun setup email. Effet grille : Onboarding 3→6, UX +2, Sécurité +0,5.
- **Action #7 (Crawl avec rendu JS — SPA) — FAITE.** `engine/scanners/browser.js` `makeRenderer()` (un Chromium partagé rend plusieurs pages), `looksLikeSpa()` (détection coquille), `crawl.js` param `renderFn` (chaque page rendue, liens JS découverts), `recon.js` rend l'accueil SPA + passe le renderer au crawl (régime payant petit; grand régime exclu pour le coût; scan gratuit mono-page inchangé). Un site React/Vue/Angular n'est plus un `<div id=root>` vide pour les agents. Dégradation gracieuse (pas de Chromium → HTML brut, aucun crash). Test `jsrender.test.mjs` 7/7 (marqueur base64 absent du brut/présent après rendu, crawl suit un lien JS), vérifié local avec vrai Chromium. Effet grille : Profondeur 7→8.
- **Action #6 (Documentation) — FAITE.** `/docs/` en ligne : démarrer, gratuit vs complet, CLI, MCP (Claude Code/Cursor), action GitHub, Slack/Jira, audit code+prod, comptes & équipe, **référence API réelle** (toutes les routes du serveur), auto-hébergement (variables d'env), les 15 agents, lecture d'un constat (verdicts). Sommaire sticky, brand-consistent, page auto-auditée propre. Lien footer + sitemap. Effet grille : Support 2→6.
- **Bonus qualité (trouvé en auto-auditant la doc) : faux positif de contraste corrigé.** L'agent contraste rejetait `var()` et retombait sur un fond blanc → fausses alertes sur **tout site dark-mode à variables CSS** (une large part du web moderne). `contrast.js` résout désormais les variables CSS. Test `contrast.test.mjs` 5/5. Renforce la promesse "zéro faux positif". Effet grille : Fiabilité/Défendabilité +0,5.
- **Score global recalculé : ~6,5/10** (Support +4, Fiabilité +0,5 sur leurs poids). Panoptic dépasse désormais Semrush (~6,4) et Siteimprove (~6,8 le talonne) sur la capacité. **Verrou restant unique côté code : le PAIEMENT** (Stripe, besoin des clés). Hors code, le seul qui compte vraiment : **des clients.**

## Distribution (chantier structurel #1 : traction) — démarré

- **Boucle produit virale — FAITE.** Chaque scan gratuit peut devenir un canal d'acquisition. `netlify/functions/share.mjs` (POST /api/share → stocke le résultat dans Netlify Blobs, **correctifs retirés = gating respecté**, renvoie une URL publique + un badge), `report-public.mjs` (GET /r/:id → rapport diagnostic public + **Open Graph** pour un aperçu social riche « Audit Panoptic de X - 82/100 » + bandeau viral CTA), `badge.mjs` (GET /badge/:id.svg → badge « Panoptic score/100 » embarquable = backlink + preuve sociale). Bouton « Partager » dans la console (lien copiable + aperçu badge + code d'embed). **Vérifié live** : partage OK, /r/:id rend l'OG + le bandeau + garde le correctif verrouillé (0 fuite), /badge/:id.svg rend le SVG. C'est de la croissance product-led intégrée au produit, pas de la pub. Renforce la Défendabilité (moat qui grandit avec chaque scan).
- **SEO comparatif — FAIT.** Pages qui captent l'intention « X vs Y » (le trafic le plus qualifié en SaaS B2B : quelqu'un qui compare achète). `build-comparatif.mjs` génère un hub `/comparatif/` + 4 pages (`/comparatif/semrush/`, `/screaming-frog/`, `/lumar/`, `/squirrelscan/`) avec tableau de capacités 12 lignes, FAQ (JSON-LD FAQPage), et positionnement honnête : **aucun chiffre concurrent inventé** — chaque page dit là où le rival est meilleur (Semrush = suite marketing + backlinks, Screaming Frog = crawl desktop massif à 199 £/an, Lumar = entreprise) et où Panoptic gagne (code + prod, RGPD, correctifs €, vérification anti-faux-positif, MCP/CLI). Prix cités uniquement quand publics et étiquetés « tarif public » (Semrush ~140 $/mois, Screaming Frog 199 £/an) ; « non communiqué » pour squirrelscan. **Auto-audité par notre propre agent a11y : 5/5 pages propres** (le seul défaut trouvé — le point « absent » du tableau à 1.42:1 — a été corrigé à `--dim` 6.5:1, dogfooding réel). Lien « Comparatifs » ajouté au footer des 6 langues, 5 URLs au sitemap. **Vérifié live** : les 5 pages rendent 200 sur panopticaudit.com.
- **Reste, hors code (le vrai) :** transformer ces boucles en clients — outbound agences, Product Hunt (les 2 autres motions, à ma disposition dès que tu veux). Le produit est prêt ; il lui faut de la distribution répétée et des premiers utilisateurs.
